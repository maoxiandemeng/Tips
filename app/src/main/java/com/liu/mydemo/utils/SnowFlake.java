...
public class SnowFlake {
    ...
    public static SnowFlake create(int width, int height, Paint paint) {
        ...
        float angle = getRandom(ANGLE_SEED) / ANGLE_SEED * ANGE_RANGE + HALF_PI - HALF_ANGLE_RANGE;
        float increment = getRandom(INCREMENT_LOWER, INCREMENT_UPPER);
        float flakeSize = getRandom(FLAKE_SIZE_LOWER, FLAKE_SIZE_UPPER);
    ...
    private void reset(int width) {
        position.x = getRandom(width);
        position.y = (int) (-flakeSize - 1);
        angle = getRandom(ANGLE_SEED) / ANGLE_SEED * ANGE_RANGE + HALF_PI - HALF_ANGLE_RANGE;
    }
    ...
    public static float getRandom(float upper) {
        return new Random().nextFloat() * upper;
    }
